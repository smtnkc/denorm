df.probe.map <- as.data.frame(read.csv("IMPORT/PROBE_MAP.csv", header = TRUE, stringsAsFactors = FALSE))
df.gpl <- as.data.frame(read.csv("IMPORT/GPL.csv", header = TRUE, stringsAsFactors = FALSE))
df.probe.map <- FormatDf(df.probe.map)
df.gpl <- FormatDf(df.gpl)
df.gpl.mapped <- MapProbes(df.gpl, df.probe.map)

DenormRunner <- function(df.gpl.mapped, range) {
  for(i in range) {
    state <- i
    df.series <- as.data.frame(read.csv("IMPORT/SERIES.csv", header = TRUE, stringsAsFactors = FALSE))[, c(1, 1 + state)]
    ...
    cat("df.clean.raw file size =", nrow(df.clean.raw), "x", ncol(df.clean.raw), "\n")
    cat("df.clean.gpl file size =", nrow(df.clean.gpl), "x", ncol(df.clean.gpl), "\n")
...
DenormRunner(df.gpl.mapped, range = c(1:1)) # Running for 35 samples takes ~ 1 hour